...
public class MLoggerSettingsProvider : SettingsProvider
{
    ...
    {
        public static readonly GUIContent LogPathLabel = new GUIContent("Log Path", "Path to the log file");

        public static readonly GUIContent MaxFileSizeLabel =
            new GUIContent("Max File Size (MB)", "Maximum size of each log file in megabytes");

        public static readonly GUIContent MaxFilesLabel =
            new GUIContent("Max Files", "Maximum number of log files to keep");

        public static readonly GUIContent AsyncModeLabel =
            new GUIContent("Async Mode", "Use asynchronous logging for better performance");

        public static readonly GUIContent ThreadPoolSizeLabel =
            new GUIContent("Thread Pool Size", "Number of threads in the async thread pool");

        public static readonly GUIContent MinLogLevelLabel =
            new GUIContent("Min Log Level", "Minimum log level to record");

        public static readonly GUIContent AutoInitializeLabel =
            new GUIContent("Auto Initialize", "Automatically initialize logger when Unity starts");

        public static readonly GUIContent AlsoLogToUnityLabel =
            new GUIContent("Also Log to Unity", "Also output logs to Unity console");
    }
    ...
    public override void OnGUI(string searchContext)
    {
        ...
        {
            var path = EditorUtility.SaveFilePanel("Select Log File", Path.GetDirectoryName(newConfig.logPath), "game",
                "log");
        ...
        if (GUILayout.Button("Flush Logs", GUILayout.Height(30)))
        {
            ...
            {
                EditorUtility.DisplayDialog("Not Initialized", "MLogger is not initialized. Logs cannot be flushed.",
                    "OK");
            }
        }
